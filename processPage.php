<?php 
	...
	$res = mysqli_query($mysqli, "SELECT * FROM  process WHERE ID=$processid");
	if (!$res) {
   		header("Location: 404.php");//没有找到结果，指向404页面
	}
	else{
		mysqli_query($mysqli, "UPDATE `process` SET`viewTimes`=`viewTimes`+1 WHERE `id`=$processid");
	}
				...
				<div id="newsRecommand">
					...
					<ul>
						<?php 
							...
							$otherNotices = mysqli_query($mysqli, "SELECT id,title,author FROM  process order by postTime asc limit 5");
							while ($line=$otherNotices->fetch_assoc()) 
							{
								echo "<li><a href=\"processPage.php?processid=".$line["id"]."\"><p>".$line["title"]."</p></a></li>";
							}
							$otherNotices->close();
							$mysqli->close();
						 ?>
