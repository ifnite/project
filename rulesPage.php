<?php 
	...
	$res = mysqli_query($mysqli, "SELECT * FROM  rules WHERE ID=$rulesid");
	if (!$res) {
   		header("Location: 404.php");//没有找到结果，指向404页面
	}
	else{
		mysqli_query($mysqli, "UPDATE `rules` SET`viewTimes`=`viewTimes`+1 WHERE `id`=$rulesid");
	}
				...
				<div id="newsRecommand">
					...
					<ul>
						<?php 
							...
							$otherNotices = mysqli_query($mysqli, "SELECT id,title,author FROM  rules order by postTime asc limit 5");
							while ($line=$otherNotices->fetch_assoc()) 
							{
								echo "<li><a href=\"rulesPage.php?rulesid=".$line["id"]."\"><p>".$line["title"]."</p></a></li>";
							}
							$otherNotices->close();
							$mysqli->close();
						 ?>
